...
#include <laughlin_uw_plugins/OWTT.h>

namespace gazebo
{
  class OWTTPlugin : public SensorPlugin
  {
    /// \brief Constructor
    public: OWTTPlugin();

    /// \brief Destructor
    public: virtual ~OWTTPlugin();
    ...
    private: PubQueue<laughlin_uw_plugins::OWTT>::Ptr pub_Queue;

    /// \brief ros message
    private: laughlin_uw_plugins::OWTT owtt_msg_;
    ...
    private: double tempDouble_;

    /// \brief Gaussian noise
    private: double rngAccuracy_;

    private: double rng_std_dev_;

    /// \brief range measurement
    ...
    private: ros::CallbackQueue owtt_queue_;
    private: void OWTTQueueThread();
    private: boost::thread callback_queue_thread_;

    //tdma subscriber to fire off range measurements
    private: ros::Subscriber tdma_sub_;
    private: std::string tdma_topic_;
    private: int beacon_number_;
    private: bool make_measurement_;
    private: void tdmaCallback(const std_msgs::Int16::ConstPtr& tdma);
    ...
    private: Eigen::Matrix4d g_Inertial_OWTTBeacon, g_Inertial_OWTTDunker;
